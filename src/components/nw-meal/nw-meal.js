...
function MealController($scope, $element, backend, utils) {
	...
	if (backend.getParam("site.externalStorageBaseUrl")) {
		vm.mealImagesPath =
			backend.getParam("site.externalStorageBaseUrl") + "/img/content/meal";
	} else {
	...
	function updateMealMenu() {
		vm.mealMenu =
			vm.service.itemsByPassengerSegments[vm.selectedPassenger][
				vm.selectedFlight
			];
	...
	function removePassengerFlightMeal(passengerNum, flightNum) {
		if (
			!vm.locked &&
			!backend.checkServiceRemovalProhibited("meal", passengerNum, flightNum)
		) {
			backend.removeExtraService({
				code: "meal",
